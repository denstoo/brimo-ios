...
Feature: Listrik
  ...
  Scenario Outline: User pay electricity bill with CA
    ...
    When I confirm electricity transaction with <detail>
    When I validate my pin with <pin> before transaction
    Then Transaction success

    Examples: 
      | username   | password   | pin 	 | listrikType	| listrikNumber		| amount	| detail			| decision	| name			| debit	| condition	|
      | bribri0001 | Jakarta123 | 123457 | Tagihan			| 547101010497		| 10000		| listrik			| Save			| Tagihan 1	| RATRI	|	NEW				|