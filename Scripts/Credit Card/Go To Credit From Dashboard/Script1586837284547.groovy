...
if (condition.toString() == 'NEW') {
    CustomKeywords.'database.methods.executeUpdate'(('DELETE FROM tbl_history_payment_cc WHERE username = "' + username.toString()) + 
        '"')

    CustomKeywords.'database.methods.executeUpdate'(('DELETE FROM tbl_list_payment WHERE username = "' + username.toString()) + 
        '" and payment_type_id = "21"')
...
Mobile.verifyElementExist(findTestObject('Feature Dashboard Home/XCUIElementTypeStaticText - Kartu Kredit'), 0)

Mobile.tap(findTestObject('Feature Dashboard Home/XCUIElementTypeStaticText - Kartu Kredit'), 0)

if (Mobile.verifyElementNotExist(findTestObject('Credit Form/XCUIElementTypeOther - Kartu Kredit'), 5, FailureHandling.OPTIONAL) == 
true) {
...
Mobile.verifyElementExist(findTestObject('Credit Form/XCUIElementTypeOther - Kartu Kredit'), 0)
